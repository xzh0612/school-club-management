...
public interface ApprovalMapper {
    ...
    @Update("UPDATE approvals SET status = #{status}, approver_id = #{approverId}, comments = #{comments} WHERE approval_id = #{approvalId}")
    ...
    @Select("SELECT ap.*, u.real_name as applicant_name, au.real_name as approver_name, " +
            ...
            "LEFT JOIN clubs c ON ap.type = 'club_creation' AND ap.related_id = c.club_id " +
            "LEFT JOIN activities a ON ap.type = 'activity_application' AND ap.related_id = a.activity_id " +
            "WHERE ap.status = 'pending' LIMIT #{offset}, #{limit}")
    ...
    @Select("SELECT ap.*, u.real_name as applicant_name, au.real_name as approver_name, " +
            ...
            "LEFT JOIN clubs c ON ap.type = 'club_creation' AND ap.related_id = c.club_id " +
            "LEFT JOIN activities a ON ap.type = 'activity_application' AND ap.related_id = a.activity_id " +
            "WHERE ap.type LIKE CONCAT('%',#{keyword},'%') OR ap.comments LIKE CONCAT('%',#{keyword},'%') LIMIT #{offset}, #{limit}")
