...
INSERT INTO recruitments(club_id, title, requirements, description, quota, start_time, end_time, status)
SELECT @club_empty_id, '空白社团边界招新', '暂无。', '用于测试 0 申请、0 成员的招新边界。', 1, DATE_ADD(NOW(), INTERVAL 1 DAY), DATE_ADD(NOW(), INTERVAL 10 DAY), 'inactive'
WHERE NOT EXISTS (SELECT 1 FROM recruitments WHERE club_id = @club_empty_id AND title = '空白社团边界招新');
UPDATE recruitments SET requirements = '暂无。', description = '用于测试 0 申请、0 成员的招新边界。', quota = 1, start_time = DATE_ADD(NOW(), INTERVAL 1 DAY), end_time = DATE_ADD(NOW(), INTERVAL 10 DAY), status = 'inactive' WHERE club_id = @club_empty_id AND title = '空白社团边界招新';
...
INSERT INTO announcements(title, content, publisher_id, target_type, target_id, status, publish_time, is_top, view_count)
SELECT '归档公告边界数据', '用于测试 archived 状态。', @admin_id, 'all', NULL, 'archived', NOW(), 0, 1
WHERE NOT EXISTS (SELECT 1 FROM announcements WHERE title IN ('撤销公告边界数据', '归档公告边界数据'));
UPDATE announcements SET title = '归档公告边界数据', content = '用于测试 archived 状态。', publisher_id = @admin_id, target_type = 'all', target_id = NULL, status = 'archived', is_top = 0, view_count = 1 WHERE title IN ('撤销公告边界数据', '归档公告边界数据');
